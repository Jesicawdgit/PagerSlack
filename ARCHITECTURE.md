...
> **Superseded:** the seeded-email lookup described below was later replaced by a team-scoped round-robin picker (`pickAssigneeForLevel` in `escalationService.js`, cursor in `Team.rotation`), and `SEEDED_USER_EMAILS` was removed — see `DECISIONS.md` › "Dynamic roles + round-robin escalation". The two bugs below are kept as history; the root cause (assuming a role is unique) no longer exists in the code.
